---
title: "TableAnswerConfig"
second_title: "Aspose.OMR 适用于 .NET API 参考"
description: "TableHeaderConfig./tableheaderconfig 的元素。表示列标题。"
type: docs
weight: 530
url: /zh/net/aspose.omr.generation.config.elements.table/tableanswerconfig/
---
## TableAnswerConfig class

[`TableHeaderConfig`](../tableheaderconfig) 的元素。表示列标题。

```csharp
public class TableAnswerConfig : BaseConfig
```

## 构造函数

| 名称 | 描述 |
| --- | --- |
| [TableAnswerConfig](tableanswerconfig)() | 默认构造函数。 |

## 属性

| 名称 | 描述 |
| --- | --- |
| override [Name](../../aspose.omr.generation.config.elements.table/tableanswerconfig/name) { get; set; } | 列标题的标题 |
| override [Type](../../aspose.omr.generation.config.elements.table/tableanswerconfig/type) { get; set; } | OMR 元素的类型。JSON 序列化所需的字段。 |

### 另请参阅

* class [BaseConfig](../../aspose.omr.generation.config/baseconfig)
* namespace [Aspose.OMR.Generation.Config.Elements.Table](../../aspose.omr.generation.config.elements.table)
* assembly [Aspose.OMR](../../)

<!-- 请勿编辑：由 xmldocmd 为 Aspose.OMR.dll 生成 -->
