---
title: TopPadding
second_title: Aspose.OMR for .NET API Referansı
description: Piksel miktarı. Öğenin üzerine fazladan boşluk eklemek için kullanılabilir. Veya negatif bir değer ayarlayarak mevcut alanı kaldırınörn. 40
type: docs
weight: 50
url: /tr/net/aspose.omr.generation.config.elements.parents/verticalchoiceboxconfig/toppadding/
---
## VerticalChoiceBoxConfig.TopPadding property

Piksel miktarı. Öğenin üzerine fazladan boşluk eklemek için kullanılabilir. Veya negatif bir değer ayarlayarak mevcut alanı kaldırın(örn. -40)

```csharp
public int TopPadding { get; set; }
```

### Ayrıca bakınız

* class [VerticalChoiceBoxConfig](../../verticalchoiceboxconfig)
* ad alanı [Aspose.OMR.Generation.Config.Elements.Parents](../../verticalchoiceboxconfig)
* toplantı [Aspose.OMR](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.OMR.dll -->
