---
title: AnswersString
second_title: Référence de l'API Aspose.OMR pour .NET
description: Obtient ou définit les chaînes de réponse pour lélément ChoiceBox
type: docs
weight: 20
url: /fr/net/aspose.omr.generation.config.elements/choiceboxconfig/answersstring/
---
## ChoiceBoxConfig.AnswersString property

Obtient ou définit les chaînes de réponse pour l'élément ChoiceBox

```csharp
public string AnswersString { get; set; }
```

### Voir également

* class [ChoiceBoxConfig](../../choiceboxconfig)
* espace de noms [Aspose.OMR.Generation.Config.Elements](../../choiceboxconfig)
* Assemblée [Aspose.OMR](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.OMR.dll -->
