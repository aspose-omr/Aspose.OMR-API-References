---
title: Name
second_title: Справочник по Aspose.OMR для .NET API
description: Имя массива пузырьков. Не участвует в отображении или генерации элемента
type: docs
weight: 70
url: /ru/net/aspose.omr.generation.config.elements.customanswersheet/bubblearrayconfig/name/
---
## BubbleArrayConfig.Name property

Имя массива пузырьков. Не участвует в отображении или генерации элемента

```csharp
public override string Name { get; set; }
```

### Смотрите также

* class [BubbleArrayConfig](../../bubblearrayconfig)
* пространство имен [Aspose.OMR.Generation.Config.Elements.CustomAnswerSheet](../../bubblearrayconfig)
* сборка [Aspose.OMR](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.OMR.dll -->
